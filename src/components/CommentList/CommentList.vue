...
<template>
  ...
  <replied-comment-modal
    ref="commentModalRef" :comment-id="currentClickedComment?.commentId" :comment-placeholder="commentPlaceholder"
    :resource-id="resourceId" :type="type" :update-comment-list="handleUpdateCommentList"
    title="回复评论"
  />
  ...
  <div v-for="(item,index) in list" :key="item.id" class="flex mt-5">
    <n-avatar :size="50" :src="item.user.avatarUrl" round />
    <div class="flex-1 pb-5 ml-4 border-0 border-b border-gray-200 dark:border-gray-200/20 border-solid">
      ...
      <div class="flex justify-between mt-2 text-xs opacity-60">
        ...
        <div class="flex items-center">
          ...
          <div
            class="pl-4 border-gray-300 dark:border-gray-300/50 cursor-pointer flex-items-center border-left"
            @click="handleClickComment(index)"
          >
